...
class PlayerokAuth:
    ...
    async def __aenter__(self):
        ...
        try:
            ...
            await self._page.goto("https://playerok.com/login", wait_until="domcontentloaded", timeout=30000)
            await asyncio.sleep(3)
            logger.info(f"Страница загружена: {self._page.url}")
        except Exception as e:
            logger.warning(f"Не удалось открыть страницу: {e}")
        return self
    ...
    async def _graphql(self, query: str, variables: dict = None, operation: str = None, token: str = None) -> tuple:
        payload = {"query": query}
        if variables:
    ...
    async def send_code(self, email: str) -> tuple[bool, str]:
        ...
        """
        data, error = await self._graphql(query, {"email": email}, operation="getEmailAuthCode")
        if error:
            return False, error
        if data is not None:
    ...
    async def verify_code(self, email: str, code: str) -> tuple:
        ...
        """
        data, error = await self._graphql(
            query,
            {"input": {"code": code, "email": email}},
            operation="checkEmailAuthCode"
        )
        if error:
            return None, error

        if data and "checkEmailAuthCode" in data:
            user = data["checkEmailAuthCode"]
            # Токен приходит в куках — достанем его
            cookies = await self._context.cookies()
            token = None
            for c in cookies:
                if "token" in c["name"].lower() or "auth" in c["name"].lower() or "session" in c["name"].lower():
                    token = c["value"]
                    logger.info(f"Найден куки токен: {c['name']}")
                    break
            if not token:
                # Попробуем взять из localStorage
                token = await self._page.evaluate("() => localStorage.getItem('token') || localStorage.getItem('authToken') || ''")
            return token or "cookie_auth", user
