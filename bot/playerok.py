...
class PlayerokClient:
    ...
    async def _get_persisted(self, operation: str, variables: dict, sha256hash: str):
        params = {
            ...
            "extensions": json.dumps({"persistedQuery": {"version": 1, "sha256Hash": sha256hash}}, separators=(',', ':')),
        }
        for attempt in range(3):
            try:
                async with AsyncSession(impersonate="chrome") as s:
                    r = await s.get(PLAYEROK_API, params=params, headers=_make_headers(token=self.token, operation=operation), timeout=15)
                    if r.status_code != 200:
    ...
    async def get_orders(self, first: int = 10):
        """Получить все заказы (не только новые)"""
    ...
    async def get_chat_messages(self, chat_id: str, first: int = 10):
        variables = {
            "pagination": {"first": first},
        ...
        return await self._get_persisted(
            ...
            "587e8a28b6a2487232d82abed32093367609d57a05601246d09926f4c83deb67"
        )
    ...
    async def get_all_my_items(self, first: int = 50):
        """Получить все лоты включая неактивные"""
        user_id = await self._get_user_id()
        if not user_id:
            return None
        variables = {
            ...
            "filter": {
                "userId": user_id,
                "status": ["APPROVED", "PENDING_MODERATION", "PENDING_APPROVAL", "REJECTED", "CLOSED"],
                "withOfficial": False
            },
            "showForbiddenImage": True
        }
    ...
    async def send_message(self, chat_id: str, text: str):
        ...
        mutation sendMessage($input: SendMessageInput!) {
            sendMessage(input: $input) { id text createdAt __typename }
        }
        """
    ...
    async def bump_item(self, item_id: str):
        query = """
        mutation bumpItem($id: ID!) {
            bumpItem(id: $id) { id priorityPosition __typename }
        }
        """
    ...
    async def restore_item(self, item_id: str):
        ...
        mutation restoreItem($id: ID!) {
            restoreItem(id: $id) { id status __typename }
        }
        """
