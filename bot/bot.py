...
async def show_profile(query, db_user: dict, token: str):
    ...
    client = PlayerokClient(token, db_user.get("playerok_user_id"))
    profile = await client.get_profile()

    balance = "—"
    unread = "—"
    if profile:
        if isinstance(profile.get("balance"), dict):
...
async def show_orders(query, token: str, user_id: str):
    from playerok import PlayerokClient
    await query.edit_message_text("⏳ Загружаю заказы...")
    client = PlayerokClient(token, user_id)
    data = await client.get_orders()
    if not data or "deals" not in data:
        text = "❌ Не удалось получить заказы"
    else:
        nodes = data["deals"].get("edges", [])
        if not nodes:
            text = "📦 Заказов нет"
        else:
            text = "📦 *Заказы:*\n\n"
            for edge in nodes[:10]:
                o = edge.get("node", {})
                buyer = o.get("user", {}).get("username", "—")
                item = o.get("item", {})
...
async def show_chats(query, token: str, user_id: str):
    from playerok import PlayerokClient
    ...
    if not data or "userChats" not in data:
        ...
        nodes = data["userChats"].get("edges", [])
        if not nodes:
            text = "💬 Чатов нет"
        else:
            text = "💬 *Активные чаты:*\n\n"
            for edge in nodes[:10]:
                ...
                text += f"👤 *{buyer}*{unread_str} — {item_name}\n\n"
    keyboard = [[InlineKeyboardButton("🔙 Назад", callback_data="back")]]
    await query.edit_message_text(text, reply_markup=InlineKeyboardMarkup(keyboard), parse_mode="Markdown")
...
async def show_items(query, token: str, user_id: str):
    from playerok import PlayerokClient
    ...
    if not data or "items" not in data:
        ...
        if not nodes:
            ...
            text = f"🛒 *Мои лоты:* {len(nodes)} шт\n\n"
            for edge in nodes[:15]:
                item = edge.get("node", {})
