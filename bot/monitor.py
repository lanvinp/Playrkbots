...
class MultiUserMonitor:
            ...
            if data and "deals" in data:
                for edge in data["deals"].get("edges", []):
                    ...
                    if order_id and order_id not in self.known_orders[telegram_id]:
                        ...
                        # Уведомляем только если это не первая загрузка
                        if len(self.known_orders[telegram_id]) > 1:
                            await self.notify_new_order(telegram_id, order, client, user)

            # ── Новые сообщения ──
            chats_data = await client.get_chats(first=10)
    ...
    async def notify_new_order(self, telegram_id: str, order: dict, client, user: dict):
        ...
        if full_user and full_user.get("auto_reply_enabled"):
            reply_text = full_user.get("auto_reply_text", "Спасибо за заказ! Выполняю как можно скорее.")
            reply_text = reply_text.replace("{username}", buyer).replace("{item}", name)
            chats_data = await client.get_chats(first=20)
            if chats_data and "userChats" in chats_data:
                for edge in chats_data["userChats"].get("edges", []):
                    chat = edge.get("node", {})
    ...
    async def notify_new_message(self, telegram_id: str, chat: dict, client, user: dict):
        ...
        # Находим собеседника (не себя)
        buyer = "—"
        for p in participants:
            username = p.get("username", "")
            if username and username != user.get("playerok_username", ""):
                buyer = username
                break
        if buyer == "—" and participants:
            buyer = participants[0].get("username", "—")
        ...
        try:
            await self.bot.send_message(chat_id=int(telegram_id), text=text, parse_mode="Markdown")
            await self.log(telegram_id, "new_message", details=f"От {buyer} ({unread} непрочитанных)")
        except Exception as e:
    ...
    async def do_auto_restore(self, telegram_id: str, client):
        data = await client.get_all_my_items(first=50)
        if not data or "items" not in data:
            return
        edges = data["items"].get("edges", [])
        inactive = [
            e["node"] for e in edges
            if e.get("node", {}).get("status") not in ("APPROVED", "PENDING_MODERATION", "PENDING_APPROVAL")
        ]
        restored = 0
